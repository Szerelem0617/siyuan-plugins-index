<script lang="ts">
    ...
    async function importPreset(preset: PresetSupertag) {
        try {
            ...
            const sql = `INSERT INTO "supertag-db" ("主键", "Icon Menu", "Conditional") VALUES ('${cleanTag}', '${iconMenuCmd}', '${conditionalScript}') ON CONFLICT("主键") DO UPDATE SET "Icon Menu" = EXCLUDED."Icon Menu", "Conditional" = EXCLUDED."Conditional"`;
            await runQuery(sql);
