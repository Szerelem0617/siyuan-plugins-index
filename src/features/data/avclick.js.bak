...
const { Plugin, Menu, showMessage, Dialog } = siyuan;
...
class DatabaseSyncPlugin extends Plugin {
    onload() {
        ...
        console.log("DatabaseSyncPlugin (Alt+Click Only) loaded");
    }
    ...
    getAVCell(event) {
        ...
        return event.target.closest(".av__cell");
    }
    ...
    async onClick(event) {
        ...
        const avContainer = cell.closest(".av");
        if (!row || !avContainer) return;

        let protyleInstance = null;
        try {
            const editors = siyuan.getAllEditor();
            if (editors && editors.length > 0) {
                protyleInstance = editors.find(e => {
                    return e.protyle && e.protyle.element && e.protyle.element.contains(avContainer);
                });
            }
        } catch (e) {
            console.warn("[SyncPlugin] ⚠️ Protyle lookup failed:", e);
        }
    ...
    async showSyncMenu(pos, avID, rowID, colID, avBlockID, protyleInstance, colType, colName, cell) {
        const menu = new Menu("av-sync-menu");
        
        console.log("[SyncPlugin] Menu Context:", { colType, colName });
        ...
        if (cell) {
            ...
            if (isIconCol) {
                const iconMenu = [];
                if (window.siyuan.config.editor.emoji && window.siyuan.config.editor.emoji.length > 0) {
                    window.siyuan.config.editor.emoji.slice(0, 10).forEach(unicode => {
                        iconMenu.push({
    ...
    openBuiltInImagesDialog(protyleInstance, avID, rowID, colID) {
        let html = "";
    ...
    triggerFileUpload(protyleInstance, avID, rowID, colID) {
        ...
        input.addEventListener("change", (e) => {
            ...
            fetch("/api/asset/upload", {
                ...
                if (res.code === 0 && res.data.succMap) {
                    ...
                    showMessage("上传失败: " + res.msg, -1, "error");
                }
            }).catch(err => {
                showMessage("上传出错: " + err.message, -1, "error");
            }).finally(() => {
                input.remove();
            });
        });
    ...
    openTemplateDialog(protyleInstance, avID, rowID, colID, avBlockID) {
        ...
        const listEl = dialog.element.querySelector("#template-list");
        const previewEl = dialog.element.querySelector("#template-preview");
        const inputEl = dialog.element.querySelector("#template-search-input");
        ...
        const renderList = (keyword = "") => {
            ...
            this.post("/api/search/searchTemplate", { k: keyword }).then(res => {
                let html = "";
                if (res && res.blocks && res.blocks.length > 0) {
                    res.blocks.forEach((item, index) => {
                ...
                const firstItem = listEl.querySelector(".b3-list-item");
                if (firstItem) {
        ...
        inputEl.addEventListener("keydown", (e) => {
            const currentFocus = listEl.querySelector(".b3-list-item--focus");
            if (e.key === "ArrowDown") {
                e.preventDefault();
    ...
    async updateCellValue(protyleInstance, avID, rowID, colID, newValue) {
        try {
            const avData = await this.post("/api/av/renderAttributeView", { id: avID });
            const rows = avData.view.rows || avData.rows || [];
            const row = rows.find(r => r.id === rowID);
            const cellIndex = avData.view.columns.findIndex(c => c.id === colID);
            ...
            const updateData = { id: cellData.id, type: cellType };
            ...
            showMessage(`✅ 已保存: ${newValue}`, 3000);
        } catch (e) {
            showMessage(`❌ 保存失败: ${e.message}`, -1, "error");
        }
    }
    ...
    async findChildItemIDs(sourceBlockID, allRows, columns) {
        ...
        const findRecursive = (parentId) => {
            ...
            children.forEach(child => {
                ...
                const blockCell = child.cells.find(c => c.valueType === "block");
    ...
    async syncAttribute(avID, rowID, colID, mode, avBlockID, protyleInstance) {
        try {
            showMessage("⏳ 正在同步...", 3000);
            const avData = await this.post("/api/av/renderAttributeView", { id: avID });
            const view = avData.view; const rows = view.rows || avData.rows || []; 
            const sourceRow = rows.find(r => r.id === rowID);
            const colIndex = view.columns.findIndex(c => c.id === colID);
            ...
            const sourceBlockID = sourceRow.cells.find(c => c.valueType === "block").value.block.id;
            let targetIDs = [];
            if (mode === "same-level") {
                const levelIdx = view.columns.findIndex(c => c.name === "Level");
                const targetLevel = sourceRow.cells[levelIdx]?.value?.number?.content;
                targetIDs = rows.filter(r => r.cells[levelIdx]?.value?.number?.content == targetLevel && r.id !== rowID).map(r => r.id);
            } else { targetIDs = await this.findChildItemIDs(sourceBlockID, rows, view.columns); }

            if (targetIDs.length === 0) return showMessage("未找到目标项", -1, "info");
            const ops = targetIDs.map(tid => {
                const cell = rows.find(r => r.id === tid).cells[colIndex];
                const data = JSON.parse(JSON.stringify(syncValue)); data.id = cell.id;
                return { action: "updateAttrViewCell", id: cell.id, avID, keyID: colID, rowID: tid, data };
            });
            if (avBlockID) ops.push({ action: "doUpdateUpdated", id: avBlockID, data: this.formatDate(new Date()) });
            if (protyleInstance) protyleInstance.transaction(ops);
            else await this.post("/api/transactions", { app: this.app.appId, reqId: Date.now(), transactions: [{ doOperations: ops }] });
            showMessage(`✅ 同步成功: 更新 ${targetIDs.length} 个项`, 3000);
        } catch (e) { showMessage(`❌ 同步失败: ${e.message}`, -1, "error"); }
    }
...
module.exports = DatabaseSyncPlugin;