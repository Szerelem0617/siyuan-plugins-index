...
export async function createDatabaseWithBlocks(sourceBlockIds: string[], protyle: any, silent: boolean = false) {
    ...
    try {
        ...
        const traverseWithContext = (element: Element, level: number, parentId: string | null, ancestorPath: string) => {
            for (let i = 0; i < element.children.length; i++) {
                ...
                if (type === "NodeListItem") {
                    ...
                    // Retrieve nested paragraph subdoc ID if present (generated by the Tree Transformation)
                    let subDocId = null;
                    const pChild = child.querySelector('div[data-type="NodeParagraph"]');
                    if (pChild) {
                        subDocId = pChild.getAttribute("custom-index-subdoc-id");
                    }
        ...
        const targetIds = allItems.map(item => item.subDocId).filter(id => id).map(id => `'${id}'`);
        const itemPropsMap: Record<string, { icon: string, titleImg: string }> = {};

        if (targetIds.length > 0 && (!existingAvID || iconKeyId || titleImgKeyId)) {
            const sqlStr = `SELECT id, ial, type FROM blocks WHERE id IN (${targetIds.join(",")})`;
